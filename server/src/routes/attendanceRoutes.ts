...
import { createSessionSchema, markAttendanceSchema } from '../lib/schemas/index';
...
ayAttendanceRouter.post('/sessions',      authenticateToken, requireAdmin, requireAYUnlocked, validateRequest(createSessionSchema), attCtrl.createSession);
...
ayAttendanceRouter.put('/events/:eventId/attendance',  authenticateToken, requireAdmin, requireAYUnlocked, validateRequest(markAttendanceSchema), attCtrl.saveEventAttendance);
...
router.post('/sessions/:sessionId/records', authenticateToken, requireAdmin, validateRequest(markAttendanceSchema), attCtrl.markAttendance);
